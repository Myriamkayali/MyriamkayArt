...
- **Public print pages**: the site's `#prints` section is **one product with a slideable image carousel**, not a grid of cards — `renderPrintsSection()` in index.html fetches `/api/prints`, filters to `status !== 'Private'` with at least one priced size (`purchasablePrints`), and shows the original "Fine Art Prints — Coming Soon" block only when that list is empty. `renderPrintProduct()` renders whichever painting is at `printCarouselIndex`: swipe (touch) or the prev/next arrows / dot indicators (`printsPrev()`/`printsNext()`/`printsGoTo()`) move between paintings, then size tags (`.size-tag`, `printsSelectSize()`) pick the A-size for *that* painting — changing painting resets the size to the first available one. "Order This Print →" opens the order flow pre-selected to the currently shown painting + size.
